package com.example.kotlinmessengerclone.Notifications
        ...
        if(firebaseUser!=null && sented == firebaseUser.uid){
            if(currentOnlineUser != user){
                if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
                    ...
                    sendOtherNotifications(mRemoteMessage)
                }
            }
        }
    }

    private fun sendOtherNotifications(mRemoteMessage: RemoteMessage) {
        ...
        val builder : NotificationCompat.Builder = NotificationCompat.Builder(this).setContentIntent(pendingIntent!!).setContentTitle(title!!)
            .setContentText(body).setSmallIcon(icon!!.toInt()).setSound(defaultSound).setAutoCancel(true)
